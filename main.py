    ...
    while running:
        for event in p.event.get():
            if event.type == p.QUIT:
                ...
                # --- NEW: Логіка для меню складності ---
                    ...
                    if easy_btn and easy_btn.collidepoint(location):
                        ...
                        ai = SmartAI(depth=2) # Можеш змінити depth, якщо хочеш
                        app_state = 'game'
                    elif diff_back_btn and diff_back_btn.collidepoint(location):
                        app_state = 'color_menu' # Повертаємось на крок назад
                    ...
                    # Only process board clicks if it is a human player's turn AND the game is active
                    human_turn = (gs.white_to_move and player_one) or (not gs.white_to_move and player_two)
        ...
        # ==========================================
        # ARTIFICIAL INTELLIGENCE MOVE LOGIC
        # ==========================================
        if app_state == 'game':
            game_over = getattr(gs, 'checkmate', False) or getattr(gs, 'stalemate', False)
            human_turn = (gs.white_to_move and player_one) or (not gs.white_to_move and player_two)
            ...
            if not human_turn and not game_over:
                # Змінено з find_random_move на find_greedy_move
        ...
        if app_state == 'main_menu':
            ...
            # Display the pawn promotion UI overlay
