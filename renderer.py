...
class Renderer:
    ...
    def draw_game_over(self, screen: p.Surface, text: str) -> None:
        """
        Draws a semi-transparent overlay with the game over text.
        """
        # Semi-transparent black background over the whole screen
        s = p.Surface((c.WIDTH, c.HEIGHT))
        s.set_alpha(180)
        s.fill(p.Color('black'))
        screen.blit(s, (0, 0))

        # Main text and shadow for better visibility
    ...
    def draw_difficulty_menu(self, screen: p.Surface) -> tuple:
        """
        Draws the AI difficulty selection menu for PvE mode.
        Returns rectangles for Easy, Medium, Hard, and Back buttons.
        """
        screen.fill(p.Color('dim gray'))
    ...
    def _draw_check_highlight(self, screen: p.Surface, gs: GameState, is_board_flipped: bool) -> None:
        """
        Highlights the King's square in red if the current player is in check.
        """
        if gs.in_check():
            # Scan the board to find the current player's King
            king_str = 'wK' if gs.white_to_move else 'bK'
            k_row, k_col = None, None  # <-- FIX: Changed from 'r, c' to avoid shadowing constants
            ...
            if k_row is not None and k_col is not None:
                ...
                # Now 'c' safely refers to the constants module again!
                s = p.Surface((c.SQ_SIZE, c.SQ_SIZE))
                s.set_alpha(150) # Semi-transparent red
                s.fill(p.Color('red'))
    ...
    def _draw_last_move(self, screen: p.Surface, gs: GameState, flipped: bool) -> None:
        """
        Highlights the starting and ending squares of the very last move made.
        """
        if len(gs.move_log) > 0:
            ...
            # Adjust coordinates if the board is flipped for Black's perspective
            if flipped:
                start_row, start_col = 7 - start_row, 7 - start_col
                end_row, end_col = 7 - end_row, 7 - end_col

            # Create a temporary surface that supports transparency (alpha channel)
            highlight_surface = p.Surface((c.SQ_SIZE, c.SQ_SIZE))
            highlight_surface.set_alpha(100) # 0 is fully transparent, 255 is solid
            highlight_surface.fill(p.Color('yellow')) # Yellow is standard for last moves

            # Draw the highlight on the starting square
            screen.blit(highlight_surface, (start_col * c.SQ_SIZE, start_row * c.SQ_SIZE))
            # Draw the highlight on the ending square
